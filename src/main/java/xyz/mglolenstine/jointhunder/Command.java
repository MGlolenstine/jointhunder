...
public class Command implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, org.bukkit.command.Command command, String s, String[] strings) {
        if(commandSender.hasPermission("jointhunder.admin")) {
            if (strings.length == 0) {
                help(commandSender);
                ...
                if (strings[0].equalsIgnoreCase("add")) {
                    if (strings[1].equalsIgnoreCase("player")) {
                        ...
                        plugin.saveConfig();
                        commandSender.sendMessage(ChatColor.GREEN+"Player "+strings[2]+" has been added.");
                        return true;
                        ...
                        plugin.saveConfig();
                        commandSender.sendMessage(ChatColor.GREEN+"Group "+strings[2]+" has been added.");
                        return true;
                    }
                    ...
                    if (strings[1].equalsIgnoreCase("player")) {
                        ...
                        plugin.getConfig().set("players", ss);
                        plugin.saveConfig();
                        commandSender.sendMessage(ChatColor.RED+"Player "+strings[2]+" has been removed.");
                        return true;
                    } else if (strings[1].equalsIgnoreCase("group")) {
                        ArrayList<String> ss = (ArrayList<String>) plugin.getConfig().getStringList("groups");
                        ss.remove(strings[2]);
                        plugin.getConfig().set("groups", ss);
                        plugin.saveConfig();
                        commandSender.sendMessage(ChatColor.RED+"Group "+strings[2]+" has been removed.");
                        return true;
                    }
                }
